...
#+++++++++++++++++++++Dataset files+++++++++++++++++++++++++++++++++++++++++++
def readdestination(): 
    f = open("cipher_text_file.txt","r")
    reverse=f.read()
    f.close()
       
    return reverse

def writedestination(plain):
    f = open("decipher_text_file.txt","w")
    f.write(str(plain)+"\n")
    f.close()
...
def source_file(): 
    f = open("plain_text_file.txt","r")
    genx=f.read()
    f.close()
       
    return genx

def destination_file(cryptotxt):
    f = open("cipher_text_file.txt","w")
    f.write(str(cryptotxt)+"\n")
    f.close()
...
# text cryptography text to int
def text_to_int(t):
    #forward
    r=str(t)
    
    res = ''.join(format(ord(i), '08b') for i in r)
    dec=int(res, base=2)
    return dec

 # text cryptography int to text 
def int_to_text(t):    
    ...
    a=k
    d=[a[i:i+8] for i in range(0, len(a), 8)]
    dcs=str(d)
    dc= dcs.replace("'", '').replace(',', '').replace('[','').replace(']','')
    a_binary_string = dc
...
def encrypto_fx(p):   #parameter is signal to be encrypted
    ...
    if rs<3:
        ...
        exint="{0:b}".format(int(ex))
        destination_file(exint)
        # int_txt=int_to_text(exint)
        # print(exint)
...
def decrypto_fx(p):            #parameter is signal to be decrypted
    ...
    if r<13:
        ...
        if not (z ==g )and (h==t and w==c and f==x):
            fx="Intrusion Detected!!!"
            print("TX:",ds)
            print("SOD:",g,"MEANIV:",c,"MEANFV:",x,"PLENX:",h)
            print("RSOD:",z,"RMEANIV:",w,"RMEANFV:",f,"RPLENX:",t)
                             
        elif (not (z ==g) and (h!=t and w!=c and f!=x) or ((z ==g ) and h!=t or  w!=c or  f!=x)):
            fx="Errors Detection!!!"
            print("TX:",ds)
            print("SOD:",g,"MEANIV:",c,"MEANFV:",x,"PLENX:",h)
            print("RSOD:",z,"RMEANIV:",w,"RMEANFV:",f,"RPLENX:",t)
            
        elif ((z ==g) and h==t and w==c and f==x):
            fx=int_to_text(ds)
            writedestination(fx)
            print("TX:",ds)
            print("SOD:",g,"MEANIV:",c,"MEANFV:",x,"PLENX:",h)
...
def main():
    ...
    if ((mode=="T") or (mode=="t")):
        ...
        encryptxt=readdestination()
        encrypted=int(str(encryptxt),base=2)
...
# encryption methods
    ...
    if typ=="e":
        tx=input("enter the input value:")
        txt=text_to_int(tx)
        eix=txt
        a=sys.getsizeof(eix)
        ...
        txt=input("enter the input value:")
        dix=int(txt,base=2)
        a=sys.getsizeof(dix)
