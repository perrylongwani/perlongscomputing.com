...
def pKey():
    ...
    res = ''.join(format(ord(i), '08b') for i in udKey)
    dec=int(res, 2)
    f.write(str(dec)+"\n")
    f.close()
...
def makeshift_m(p): #parameter is signal to be encrypted
    ...
    if c>9:
        c=9         #to avoid out of range data
    x=m-c           #value to shift the digits in the 1 st round data
    if x<0:
        x=m+c        #to avoid negative value error
    ...
    fm=str(md)+str(c) #random values appended
...
def encrypto_fx(p):   #parameter is signal to be encrypted
    ...
    if r<3:
        ...
        md=str(m)
        rn=md[-1]         #random value sub key
        mdt=""
        for i in range(0,len(md)-1):
            mdt=mdt+md[i] #makeshift data after removing the random value sub key
            
        v=meaniv(p)       #call mean IV function
        f=meanfv(p)       #call mean FV function
        l=plenx(p)        #call plenx function
        
        ex=str(mdt)+str(rn)+str(s)+str(f)+str(l)+str(v)#with appended six key values for decryption and intrusion/error detection function
        k=int(str(rn)+str(s)+str(f)+str(l)+str(v))
        exint=int(ex)
       ...
       #to be commented
        print("Public key",us)
        print("Private key",k)
        print("Private Key Size",sys.getsizeof(k))
        print("Public Key Size",sys.getsizeof(us))
        print("Transmitted message Size",sys.getsizeof(enc))
...
def rplenx(p):      #parameter is signal to be decrypted
    ...
    y=str(s)        #convert to string
    r=len(y)        #length of the parameter
    px=7            #standard length of added digits are 6 plus 1 randpm value
    w=y[r-px:r]     #extract last 6 digits plus 1 randpm value
    t=""            #Initialise the varible
    
    for i in range(0,px):
        t=t+w[i]
        
    rp=t[5]        # extract the plenx which is position 6(0-5)
...
def rmeaniv(p):     #parameter is signal to be decrypted
    ...
    rp=t[6]        # extract the mean IV which is position 7(0-6)
...
def rmeanfv(p):     #parameter is signal to be decrypted
    ...
    rp=t[4]        # extract the mean FV which is position 5(0-4)
...
def rrandv(p):     #parameter is signal to be decrypted
    ...
    rp=t[0]        # extract the random value which is position 1(0)
...
def rsod(p):        #parameter is signal to be decrypted
    ...
    rp=""          #Initialise the varible
    for i in range(1,4):
        rp=rp+t[i]  # extract the sod which is the second three digits of added digits
...
def rmakeshiftdata(p):    #parameter is signal to be decrypted prepared data
    ...
    r=len(y)        #length of the parameter
    px=7            #standard length of added digits are 6 plus 1 randpm value
    w=y[0:r-px]     #extract last 6 digits plus 1 randpm value
    pr=""           #Initialise the varible
    
    for i in range(0,r-px):
        pr=pr+w[i]  #reverse prepared data
...
def decryption_m(p):                #parameter is signal to be decrypted
    ...
    dk=int(rusKey())
    dix=int(s)-dk
    decrypto_tx=decrypto_fx(dix)      #call the decrypto finaliser
...
def main():
    ...
    if ((mode=="T") or (mode=="t")):
        ...
        signal=geny
        if (len(signal))>6:     #Generated signal length should not be more than 6 according the system protocol
             return 0
        else:
             encryption_m(signal)             
...
# encryption methods
    ...
    if typ=="e":
        ...
        a=sys.getsizeof(eix)
        b=sys.getsizeof(pKey())
        c=sys.getsizeof(randv())
        d=sys.getsizeof(plenx(eix))
        e=sys.getsizeof(predata(eix))
        f=sys.getsizeof(meaniv(eix))
        g=sys.getsizeof(meanfv(eix))
        h=sys.getsizeof(sod(eix))
        i=sys.getsizeof(pop_m(eix))
        j=sys.getsizeof(makeshift_m(eix))
        k=sys.getsizeof(encrypto_fx(eix))
        l=sys.getsizeof(encryption_m(eix))
